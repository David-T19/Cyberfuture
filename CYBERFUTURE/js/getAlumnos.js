document.addEventListener("DOMContentLoaded", () => {
  ...
  async function probarConexion() {
    const endpoint = "http://cybercenter.somee.com/api/Alumno/";
    const resultados = [];
  ...
  function renderizarTabla(datos) {
    ...
    datosPaginados.forEach(alumno => {
      ...
      fila.innerHTML = `
        ...
        <td>${alumno.IdPractica ?? ""}</td>
        
        <td class="text-center">
          <button value="${alumno.IdAlumno ?? ""}" class="btn btn-sm btn-warning btn-editar">Editar</button>
          <button value="${alumno.IdAlumno ?? ""}" class="btn btn-sm btn-danger btn-borrar">Eliminar</button>
        </td>
      `;
  ...
  tabla.addEventListener("click", async (event) => {
    ...
    if (target.classList.contains("btn-editar")) {
      const id = target.value;
      window.location.href = `../view Alumno/editar.html?id=${encodeURIComponent(id)}`;
      return;
    }
  });
