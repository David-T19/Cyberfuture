document.addEventListener("DOMContentLoaded", () => {
  ...
  async function probarConexion() {
    const endpoint = "http://cybercenter.somee.com/api/Factura/";
    const resultados = [];
  ...
  function renderizarTabla(datos) {
    ...
    datosPaginados.forEach(factura => {
      ...
      fila.innerHTML = `
        ...
        <td>${factura.IdAlumno ?? ""}</td>
        
        
        <td class="text-center">
          <button value="${factura.IdFactura ?? ""}" class="btn btn-sm btn-warning btn-editar">Editar</button>
          <button value="${factura.IdFactura ?? ""}" class="btn btn-sm btn-danger btn-borrar">Eliminar</button>
        </td>
      `;
  ...
  tabla.addEventListener("click", async (event) => {
    ...
    if (target.classList.contains("btn-editar")) {
      const id = target.value;
      window.location.href = `../view Factura/editar.html?id=${encodeURIComponent(id)}`;
      return;
    }
  });
