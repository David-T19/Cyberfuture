document.addEventListener("DOMContentLoaded", () => {
  ...
  async function probarConexion() {
    const endpoint = "http://cybercenter.somee.com/api/Curso/";
    const resultados = [];
    ...
    datosPaginados.forEach(curso => {
      ...
      fila.innerHTML = `
        ...
        <td class="text-center">
          <button value="${curso.IdCurso ?? ""}" class="btn btn-sm btn-warning btn-editar">Editar</button>
          <button value="${curso.IdCurso ?? ""}" class="btn btn-sm btn-danger btn-borrar">Eliminar</button>
        </td>
      `;
  ...
  tabla.addEventListener("click", async (event) => {
    ...
    if (target.classList.contains("btn-editar")) {
      const id = target.value;
      window.location.href = `../view Curso/editar.html?id=${encodeURIComponent(id)}`;
      return;
    }
  });
