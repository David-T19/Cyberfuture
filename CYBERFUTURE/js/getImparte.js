document.addEventListener("DOMContentLoaded", () => {
  ...
  async function probarConexion() {
    const endpoint = "http://cybercenter.somee.com/api/Imparte/";
    const resultados = [];
  ...
  function renderizarTabla(datos) {
    ...
    datosPaginados.forEach(imparte => {
      const fila = document.createElement("tr");
      fila.innerHTML = `
        <td>${ imparte.IdInstructor ?? ""}</td>
        <td>${ imparte.IdCurso ?? ""}</td>
        <td>${ imparte.Tematica ?? ""}</td>
           
        <td class="text-center">
            <button 
            value='${JSON.stringify({idInstructor: imparte.IdInstructor, idCurso: imparte.IdCurso})}' 
  ...
  tabla.addEventListener("click", async (event) => {
    ...
    if (target.classList.contains("btn-borrar")) {
      try {
        ...
        const idInstructor = ids.idInstructor;
        ...
        if (confirm(`¿Eliminar este registro?\nInstructor: ${idInstructor}\nCurso: ${idCurso}`)) {
          ...
          try {
            response = await fetch(`https://corsproxy.io/?url=http://cybercenter.somee.com/api/Imparte/${idInstructor}/${idCurso}`, {
              method: "DELETE"
            });
          ...
          if (!success) {
            try {
              response = await fetch("https://corsproxy.io/?url=http://cybercenter.somee.com/api/Imparte/", {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ IdInstructor: idInstructor, IdCurso: idCurso })
              });
              if (response.ok) success = true;
            } catch (e) {}
          }
          ...
          datosReales = datosReales.filter(item => 
            !(item.IdInstructor == idInstructor && item.IdCurso == idCurso)
          );
    ...
    if (target.classList.contains("btn-editar")) {
      try {
        ...
        window.location.href = `../view Imparte/editar.html?idInstructor=${encodeURIComponent(idInstructor)}&idCurso=${encodeURIComponent(idCurso)}`;
      } catch (err) {
