document.addEventListener("DOMContentLoaded", () => {
  ...
  async function probarConexion() {
    const endpoint = "http://cybercenter.somee.com/api/PracticasEmpresariales/";
    const resultados = [];
  ...
  function renderizarTabla(datos) {
    ...
    datosPaginados.forEach(practica => {
      const fila = document.createElement("tr");
      fila.innerHTML = `
        <td>${practica.IdPractica ?? ""}</td>
        <td>${practica.No_Horas ?? ""}</td>
        <td>${practica.Fecha_Inicio ?? ""}</td>
        <td>${practica.Fecha_Final ?? ""}</td>
        
        
        <td class="text-center">
          <button value="${practica.IdPractica ?? ""}" class="btn btn-sm btn-warning btn-editar">Editar</button>
          <button value="${practica.IdPractica ?? ""}" class="btn btn-sm btn-danger btn-borrar">Eliminar</button>
        </td>
      `;
  ...
  tabla.addEventListener("click", async (event) => {
    ...
    if (target.classList.contains("btn-editar")) {
      const id = target.value;
      window.location.href = `../view Practicas Empresariales/editar.html?id=${encodeURIComponent(id)}`;
      return;
    }
  });
